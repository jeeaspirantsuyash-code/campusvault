...
function getDashboardStats() {
  ...
  const avgCgpa =
    db
      .prepare(
        'SELECT ROUND(AVG(cgpa), 2) AS a FROM students'
      )
      .get().a || 0;
  ...
  const avgAttendance =
    db
      .prepare(
        'SELECT ROUND(AVG(attendance), 1) AS a FROM students'
      )
      .get().a || 0;

  const lowAttendanceCount = db
    .prepare(
      'SELECT COUNT(*) AS c FROM students WHERE attendance < 65'
    )
    .get().c;
...
function listStudents(query) {
  ...
  if (query.search) {
    clauses.push(
      '(full_name LIKE ? OR roll_no LIKE ? OR email LIKE ? OR enrollment_id LIKE ?)'
    );
  ...
  if (
    ...
    try {
      ...
      `).run(
        ...
        merged.email,
        merged.roll_no,
        merged.enrollment_id,
        merged.department,
        merged.program,
        Number(merged.year),
        merged.section,
        Number(merged.cgpa),
        Number(merged.attendance),
        merged.status,
        id
      );
...
server.listen(
  ...
  () => {
    console.log(
      `CampusVault running at http://localhost:${PORT}`
    );
