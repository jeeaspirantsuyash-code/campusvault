...
function getDashboardStats() {
  ...
  const departments = deptRows.map((d) => ({
    ...d,
    capacityPct: Math.round((d.count / maxDeptCount) * 100),
  }));
...
async function handleApi(req, res, pathname, query) {
  ...
  if (req.method === 'POST' && pathname === '/api/students') {
    ...
    try {
      ...
      const info = db.prepare(`
        INSERT INTO students (full_name, email, roll_no, enrollment_id, department, program, year, section, cgpa, attendance, status)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(body.full_name, body.email, body.roll_no, body.enrollment_id, body.department, body.program, Number(body.year), body.section, Number(body.cgpa), Number(body.attendance), status);
  ...
  if (req.method === 'PUT' && studentMatch) {
    ...
    try {
      db.prepare(`
        UPDATE students SET full_name=?, email=?, roll_no=?, enrollment_id=?, department=?, program=?, year=?, section=?, cgpa=?, attendance=?, status=?
        WHERE id=?
      `).run(merged.full_name, merged.email, merged.roll_no, merged.enrollment_id, merged.department, merged.program, Number(merged.year), merged.section, Number(merged.cgpa), Number(merged.attendance), merged.status, id);
  ...
  if (req.method === 'POST' && pathname === '/api/backups') {
    ...
    if (!body.pin || body.pin !== pinRow.value) {
      logAction('BACKUP_PIN_FAILED', {});
      return sendJson(res, 401, { error: 'Incorrect security PIN' });
    }
    ...
    const info = db.prepare('INSERT INTO backups (filename, student_count, size_bytes) VALUES (?, ?, ?)').run(filename, students.length, size);
    logAction('BACKUP_CREATED', { filename, studentCount: students.length });
    const backup = db.prepare('SELECT * FROM backups WHERE id = ?').get(info.lastInsertRowid);
  ...
  if (req.method === 'POST' && restoreMatch) {
    ...
    const pinRow = db.prepare('SELECT value FROM settings WHERE key = ?').get('backup_pin');
    if (!body.pin || body.pin !== pinRow.value) {
      logAction('RESTORE_PIN_FAILED', { backupId: id });
      return sendJson(res, 401, { error: 'Incorrect security PIN' });
    }
